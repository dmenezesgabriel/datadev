...
from pymdownx.superfences import SuperFencesException, _escape  # type: ignore
...
def fence_vegalite(source, language, class_name, options, md, **kwargs):
    ...
    if not _validateJSON(source):
        raise SuperFencesException from PluginError(
            f"Your vegalite syntax is not valid JSON. Fix:\n\n{source}"
        )
...
CUSTOM_FENCES = [
    {"name": "vegalite", "class": "vegalite", "format": fence_vegalite}
]
...
class MkdocsMaterialOptions(base.Config):
    themes_light = config_options.ListOfItems(
        config_options.Type(str), default=["default"]
    )
    themes_dark = config_options.ListOfItems(
        config_options.Type(str), default=["slate"]
    )
...
class MkDocsVegalitePlugin(BasePlugin):
    config_scheme = (
    ...
    def on_config(self, config, **kwargs):
        ...
        custom_fences = parent_config.get("pymdownx.superfences", {}).get(
            "custom_fences", {}
        )
    ...
    def on_post_build(self, config, **kwargs):
        ...
        js_file_path = os.path.join(
            js_output_base_path, "mkdocs-charts-plugin.js"
        )
        copy_file(
    ...
    def add_javascript_variables(self, html, page, config):
        ...
        path_to_homepage = os.path.relpath(
            docs_directory, os.path.dirname(page_path)
        )
